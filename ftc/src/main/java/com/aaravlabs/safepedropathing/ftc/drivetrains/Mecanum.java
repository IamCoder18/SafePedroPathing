...
import static com.aaravlabs.safepedropathing.math.MathFunctions.findNormalizingScaling;
...
public class Mecanum extends Drivetrain {
     ...
     */
    public Mecanum(SafeHardwareMap safeMap, MecanumConstants mecanumConstants) {
    ...
    public double[] calculateDrive(Vector correctivePower, Vector headingPower, Vector pathingPower, double robotHeading) {
        ...
        wheelPowers[1] = (mecanumVectorsCopy[0].getXComponent() * truePathingVectors[0].getYComponent() - truePathingVectors[0].getXComponent() * mecanumVectorsCopy[0].getYComponent()) / (mecanumVectorsCopy[0].getXComponent() * mecanumVectorsCopy[1].getYComponent() - mecanumVectorsCopy[1].getXComponent() * mecanumVectorsCopy[0].getXComponent());
    ...
    public void runDrive(double[] drivePowers) {
        for (int i = 0; i < motors.size(); i++) {
            if (Math.abs(lastMotorPowers[i] - drivePowers[i]) > motorCachingThreshold ||
                ...
                final int idx = i;
                final double power = drivePowers[i];
